...
contract Audit {
    ...
    function confirmAuditor(address _auditorAddr) external {
        if (_auditorAddr != tx.origin) {
            revert OnlyContractAreAllowed();
        }
    ...
    function removeAuditor(address _auditor) public {
        ...
        if (auditorToRemove.currentGigs > 0) {
            Auditor memory replacementAuditor = findAvailableAuditor();
    ...
    function findAvailableAuditor() internal view returns (Auditor memory) {
        Auditor memory selectedAuditor;
        uint32 earliestConfirmationTime = type(uint32).max;
    ...
    function editCurrentGigs(
        ...
        if (auditorToEdit._auditor == address(0)) {
            revert AuditorNotFound();
        }
